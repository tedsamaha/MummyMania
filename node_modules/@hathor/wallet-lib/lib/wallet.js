...
const wallet = {
  ...
  executeGenerateWallet(words, passphrase, pin, password, loadHistory) {
    ...
    let privkey = xpriv.deriveNonCompliantChild(`m/44'/${_constants.HATHOR_BIP44_CODE}'/0'/0`);
  ...
  loadAddressHistory(startIndex, count, connection = null, store = null) {
    const promise = new Promise((resolve, reject) => {
      ...
      for (var i = startIndex; i < stopIndex; i++) {
        ...
        var address = (0, _bitcoreLib.Address)(key.publicKey, _network.default.getNetwork());
  ...
  _getChangePinAccessData(accessData, oldPin, newPin) {
    ...
    const encryptedData = this.encryptData(decryptedData, newPin); // Create a new object (without mutating the old one) with the updated data
  ...
  generateNewAddress(connection = null) {
    const accessData = this.getWalletAccessData();
    const xpub = (0, _bitcoreLib.HDPublicKey)(accessData.xpubkey); // Get last shared index to discover new index

    const lastSharedIndex = this.getLastSharedIndex();
    let newIndex = lastSharedIndex + 1;
    const newKey = xpub.deriveChild(newIndex);
    const newAddress = (0, _bitcoreLib.Address)(newKey.publicKey, _network.default.getNetwork()); // Update address data and last generated indexes
  ...
  getAddressAtIndex(index) {
    ...
    const address = (0, _bitcoreLib.Address)(newKey.publicKey, _network.default.getNetwork());
  ...
  saveNewHistoryOnStorage(oldHistoryTransactions, oldAllTokens, newHistory, dataJson, connection = null, store = null) {
    ...
    if (maxIndex > lastUsedIndex && lastUsedAddress !== null) {
      ...
      if (candidateIndex > lastSharedIndex) {
        const xpub = (0, _bitcoreLib.HDPublicKey)(this.getWalletAccessData().xpubkey);
        const key = xpub.deriveChild(candidateIndex);
        const address = (0, _bitcoreLib.Address)(key.publicKey, _network.default.getNetwork()).toString();
