...
const helpers = {
  ...
  getTxType(tx) {
    if (this.isBlock(tx)) {
      if (tx.version === _constants.BLOCK_VERSION) {
        return 'Block';
      } else if (tx.version === _constants.MERGED_MINED_BLOCK_VERSION) {
        return 'Merged Mining Block';
      }
    } else {
      if (tx.version === _constants.DEFAULT_TX_VERSION) {
        return 'Transaction';
      } else if (tx.version === _constants.CREATE_TOKEN_TX_VERSION) {
   ...
   */
  isBlock(tx) {
    return tx.version === _constants.BLOCK_VERSION || tx.version === _constants.MERGED_MINED_BLOCK_VERSION;
  },
   ...
   */
  prettyValue(value) {
    const fixedPlaces = (value / 10 ** _constants.DECIMAL_PLACES).toFixed(_constants.DECIMAL_PLACES);
   ...
   */
  getServerURL() {
    return _config.default.getServerUrl();
  },
  ...
  isXpubKeyValid(xpubkey) {
    try {
      (0, _bitcoreLib.HDPublicKey)(xpubkey);
      return true;
    } catch (error) {
      return false;
    }
  ...
  getAddresses(xpubkey, startIndex, quantity, networkName) {
    ...
    if (networkName) {
      _network.default.setNetwork(networkName);
    }
    ...
    for (let index = startIndex; index < startIndex + quantity; index++) {
      ...
      const address = (0, _bitcoreLib.Address)(key.publicKey, _network.default.getNetwork());
