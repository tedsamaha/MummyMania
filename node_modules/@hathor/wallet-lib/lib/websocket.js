...
class WS extends _events.default {
  constructor({
    wsURL,
    ...
    super(); // This is the class of the Websocket. It is used to open new WebSocket
    // connections. We don't directly use it from import, so the unittests
    // can replace it by a mock.

    this.WebSocket = _isomorphicWs.default; // This is the URL of the websocket.

    this.wsURL = wsURL; // Boolean to show when there is a websocket started with the server
    ...
    this.isOnline = undefined; // Heartbeat interval in milliseconds.

    this.heartbeatInterval = heartbeatInterval; // Connection timeout.

    this.connectionTimeout = connectionTimeout; // Retry connection interval in milliseconds.

    this.retryConnectionInterval = retryConnectionInterval; // Open connection timeout.

    this.openConnectionTimeout = openConnectionTimeout; // Date of connection.

    this.connectedDate = null; // Date of latest setup call. The setup is the way to open a new connection.

    this.latestSetupDate = null; // Date of latest ping.

    this.latestPingDate = null; // Latest round trip time measured by PING/PONG.

    this.latestRTT = null; // Timer used to detected when connection is down.
  ...
  onError(evt) {
    ...
    this.onClose(); // console.log('ws error', window.navigator.onLine, evt);
  }
  /**
