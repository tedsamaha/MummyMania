...
class HathorWallet extends _events.default {
  ...
  constructor({
    ...
    if (connection.state !== _connection.default.CLOSED) {
  ...
  onConnectionChangedState(newState) {
    if (newState === _connection.default.CONNECTED) {
      _storage.default.setStore(this.store);
  ...
  getCurrentAddress({
    ...
    const addressPath = `m/44'/${_constants.HATHOR_BIP44_CODE}'/0'/0/${index}`;
    return {
      address,
      index,
      addressPath
  ...
  onTxArrived(tx, isNew) {
    ...
    const tokensBalance = this.getPreProcessedData('balanceByToken'); // we first get all tokens present in this tx (that belong to the user) and
    // the corresponding balances
  ...
  start(optionsParams = {}) {
    ...
    if (this.seed) {
      ret = _wallet.default.executeGenerateWallet(this.seed, this.passphrase, pinCode, password, false);
    } else if (this.xpriv) {
      ret = _wallet.default.executeGenerateWalletFromXPriv(this.xpriv, pinCode, false);
    } else if (this.xpub) {
      ret = _wallet.default.executeGenerateWalletFromXPub(this.xpub, false);
    } else {
  ...
  stop({
    ...
    this.walletStopped = true; // TODO Double check that we are properly cleaning things up.
