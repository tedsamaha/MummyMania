...
const tokens = {
  ...
  getTokenUID(txID, index) {
    let arr = [];
    arr.push((0, _buffer2.hexToBuffer)(txID));
    arr.push(_transaction.default.intToBytes(index, 1));
  ...
  validateTokenToAddByConfigurationString(config, uid) {
    const promise = new Promise((resolve, reject) => {
      ...
      if (tokenData === null) {
        reject(new _errors.TokenValidationError('Invalid configuration string'));
      }

      if (uid && uid !== tokenData.uid) {
        reject(new _errors.TokenValidationError(`Configuration string uid does not match: ${uid} != ${tokenData.uid}`));
      }
  ...
  validateTokenToAddByUid(uid, name, symbol) {
    const promise = new Promise((resolve, reject) => {
      ...
      if (token) {
        reject(new _errors.TokenValidationError(`You already have this token: ${uid} (${token.name})`));
      ...
      if (tokenInfo) {
        reject(new _errors.TokenValidationError(`You already have a token with this ${tokenInfo.key}: ${tokenInfo.token.uid} - ${tokenInfo.token.name} (${tokenInfo.token.symbol})`));
      } // Validate if name and symbol match with the token info in the DAG


      _wallet2.default.getGeneralTokenInfo(uid, response => {
        if (response.success) {
          if (response.name !== name) {
            reject(new _errors.TokenValidationError(`Token name does not match with the real one. Added: ${name}. Real: ${response.name}`));
          } else if (response.symbol !== symbol) {
            reject(new _errors.TokenValidationError(`Token symbol does not match with the real one. Added: ${symbol}. Real: ${response.symbol}`));
          } else {
            resolve();
          }
        } else {
          reject(new _errors.TokenValidationError(response.message));
        }
      });
    });
    return promise;
   ...
   */
  getTokens() {
    let dataToken = _storage.default.getItem('wallet:tokens');
  ...
  saveToStorage(newTokens) {
    _storage.default.setItem('wallet:tokens', newTokens);
  },
  ...
  getConfigurationString(uid, name, symbol) {
    ...
    const checksum = _transaction.default.getChecksum(_buffer.default.Buffer.from(partialConfig));
  ...
  getTokenFromConfigurationString(config) {
    ...
    const correctChecksum = _transaction.default.getChecksum(_buffer.default.Buffer.from(configWithoutChecksum));
  ...
  tokenInfoExists(name, symbol) {
    ...
    for (const token of tokens) {
      if (_helpers.default.cleanupString(token.name) === _helpers.default.cleanupString(name)) {
        return {
          token,
          key: 'name'
        };
      ...
      if (_helpers.default.cleanupString(token.symbol) === _helpers.default.cleanupString(symbol)) {
        return {
          token,
          key: 'symbol'
        };
  ...
  generateCreateTokenData(address, name, symbol, mintAmount, pin, options = {}) {
    ...
    try {
      ...
      if (e instanceof _errors.InsufficientFundsError) {
        return {
          success: false,
          message: 'Don\'t have enough HTR funds to mint this amount.'
        };
      } else {
        // Unhandled error
        throw e;
    ...
    const createTokenTxData = Object.assign(txData, {
      version: _constants.CREATE_TOKEN_TX_VERSION,
      name,
      symbol
    });
   ...
   * @inner
   */
  createToken(address, name, symbol, mintAmount, pin, options = {
    changeAddress: null
  }) {
    ...
    const sendTransaction = new _sendTransaction.default({
      transaction: _helpers2.default.createTxFromData(ret.preparedData, _network.default),
  ...
  createMintData(mintInput, token, address, amount, depositInputs, options) {
    ...
    if (!depositInputs) {
      ...
      inputs.push(...depositInputs.inputs); // create change output, if needed

      const depositAmount = this.getDepositAmount(amount);

      if (depositInputs.amount - depositAmount > 0) {
        const outputChange = _wallet.default.getOutputChange(depositInputs.amount - depositAmount, 0, {
          address: changeAddress
        });

        outputs.push(outputChange);
      } else if (depositInputs.amount - depositAmount < 0) {
        throw new _errors.InsufficientFundsError(`Not enough HTR tokens for deposit: ${depositAmount} required, ${htrInputs.inputsAmount} available`);
      }
    ...
    if (mintInput) {
      ...
      inputs.push({
        'tx_id': mintInput.tx_id,
        'index': mintInput.index,
        'token': token,
        'address': mintInput.address
      });
    ...
    if (createAnotherMint) {
      ...
      const newAddress = _wallet.default.getAddressToUse();
    ...
    if (createMelt) {
      ...
      const newAddress2 = _wallet.default.getAddressToUse();
   ...
   * @inner
   */
  createMeltData(meltInput, token, amount, createAnotherMelt, options = {
    depositAddress: null,
    changeAddress: null
  }) {
    const {
      depositAddress,
      changeAddress
    } = options; // Get inputs that sum at least the amount requested to melt

    const result = this.getMeltInputs(amount, token); // Can't find inputs to this amount

    if (result === null) return null; // First adding authority input with MELT capability that will be spent

    const authorityInput = {
      'tx_id': meltInput.tx_id,
      'index': meltInput.index,
      'token': token,
      'address': meltInput.address
    ...
    if (result.inputsAmount > amount) {
      ...
      outputs.push(_wallet.default.getOutputChange(result.inputsAmount - amount, 1, {
        address: changeAddress
      }));
    }
    ...
    if (withdrawAmount > 0) {
      ...
      outputs.push(_wallet.default.getOutputChange(withdrawAmount, 0, {
   ...
   * @inner
   */
  generateMeltData(meltInput, token, amount, pin, createAnotherMelt, options = {
    depositAddress: null,
    changeAddress: null
  }) {
    // Get melt data
    ...
    if (!newTxData) {
      return {
        success: false,
        message: 'There aren\'t enough inputs to melt.'
      };
    }
   ...
   * @inner
   */
  meltTokens(meltInput, token, amount, pin, createAnotherMelt, options = {
    depositAddress: null,
    changeAddress: null
  }) {
  ...
  getMeltInputs(amount, token) {
    const data = _wallet.default.getWalletData(); // If wallet has no data yet, return null
    ...
    const inputs = [];
    let inputsAmount = 0; // Get history for this token

    const filteredHistory = _wallet.default.filterHistoryTransactions(data.historyTransactions, token, false);
    ...
    for (const tx of filteredHistory) {
      ...
      for (const [index, txout] of tx.outputs.entries()) {
        ...
        if (txout.spent_by === null && txout.token === token && _wallet.default.isAddressMine(txout.decoded.address, data)) {
          inputs.push({
            'tx_id': tx.tx_id,
            'index': index,
            'token': token,
  ...
  createDelegateAuthorityData(txID, index, addressSpent, token, address, createAnother, type) {
    ...
    const input = {
      ...
      'index': index,
      'token': token,
      'address': addressSpent
    }; // Setting the output value delegated, depending on the authority type

    const outputValue = type === 'mint' ? _constants.TOKEN_MINT_MASK : _constants.TOKEN_MELT_MASK; // Output1: Delegated output
  ...
  handlePrepareData(data, pin, options) {
    try {
      const preparedData = _transaction.default.prepareData(data, pin, options);
  ...
  handleSendTransaction(data) {
    const sendTransaction = new _sendTransaction.default({
      transaction: _helpers2.default.createTxFromData(data, _network.default),
      network: _network.default
  ...
  getTokenIndex(tokens, uid) {
    ...
    if (uid === _constants.HATHOR_TOKEN_CONFIG.uid) {
      ...
      const tokensWithoutHathor = this.filterTokens(tokens, _constants.HATHOR_TOKEN_CONFIG);
  ...
  getMintDepositInfo(mintAmount, options = {}) {
    ...
    const htrInputs = _wallet.default.getInputsFromAmount(data.historyTransactions, depositAmount, _constants.HATHOR_TOKEN_CONFIG.uid);

    if (htrInputs.inputsAmount < depositAmount) {
      throw new _errors.InsufficientFundsError(`Not enough HTR tokens for deposit: ${_helpers.default.prettyValue(depositAmount)} required, ${_helpers.default.prettyValue(htrInputs.inputsAmount)} available`);
    }

    if (htrInputs.inputsAmount > depositAmount) {
      // Need to create change output
      const outputChange = _wallet.default.getOutputChange(htrInputs.inputsAmount - depositAmount, 0, {
        address: changeAddress
      });
    ...
    return {
      'inputs': htrInputs.inputs,
      'inputsAmount': htrInputs.inputsAmount,
      'outputs': outputs
    };
  },
  ...
  getDepositPercentage() {
    if (this._depositPercentage === null) {
      ...
      return _constants.TOKEN_DEPOSIT_PERCENTAGE;
    }
   ...
   */
  isHathorToken(uid) {
    return uid === _constants.HATHOR_TOKEN_CONFIG.uid;
  },
